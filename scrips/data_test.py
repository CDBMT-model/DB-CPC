...
def get_query_max(meta_datas):
    ...
    for i in trange(len(meta_datas)):
        if(len(meta_datas.iloc[i]['query']) == 1):
            q_lens.append(len(meta_datas.iloc[i]['query'][0].split(' ')))
            #print(q_lens[-1])
...
def main():
    ...
    with open(os.path.join(config.processed_path, 'dataset_time_'+data_name+'.bin'), 'rb') as f:
        data_set = pickle.load(f)

    data_set.init_dataset(review_datas)
    data_set.init_sample_table()
